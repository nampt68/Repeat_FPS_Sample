...
public struct GameTime
{
    ...
    public int tickRate //초당 틱, 60
    {
        get { return m_tickRate; }
        set
        {
            m_tickRate = value;
            tickInterval = 1.0f / m_tickRate; //0.0166초 if tickrate is 60
        }
    }
    ...
    public float tickDuration;          // Duration of current tick 지금 6틱이라면 이후 얼마 시간이 지났는지를 나타내는 값. 초 단위이다.
    ...
    {
        get { return tickDuration / tickInterval; } //0.몇 tick
    }
    ...
    {
        return (this.tick - tick) * tickInterval + tickDuration; // tick 이후 얼마의 시간이 지났는지
    }
...
public class Game : MonoBehaviour
{
    ...
    //MovableCube Prefeb이 할당되어 있다. WeakAssetReference는 Prefeb을 할당하기 위한 용도.
    //추후 Prefeb을 Load 한다.
    ...
    //Color에 Enum을 넘겨 주면, Enum의 각 이름이 Inspector상 변수로 할당됨.
